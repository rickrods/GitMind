...
import { saveIssueAnalysis, getIssueAnalyses } from '@/app/actions/actions';
import { GitHubIssue, AIProposal } from '@/types';
import { useRepo } from '@/components/providers/RepoContext';

export default function Issues() {
  const { currentRepo, githubService, geminiApiKey, token, setError } = useRepo();
  ...
  const handleAnalyze = async (issue: GitHubIssue, feedback?: string) => {
    ...
    try {
      ...
      const result = await analyzeIssue(issue, currentRepo, structureString, geminiApiKey, feedback);
  ...
  const handleCreateProposedPR = async () => {
    if (!proposalDetails || !currentRepo || !selectedIssue || !token) {
    ...
    try {
      const response = await fetch('/api/propose-fix', {
        ...
        body: JSON.stringify({
          ...
          headBranch: currentRepo.default_branch, // In issues case, we branch from default
          fix: proposalDetails,
        }),
      });
  ...
  return (
    <div className="p-8 space-y-6">
      <header className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white">Issue Manager: <span className="text-github-blue">{currentRepo.name}</span></h2>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <h4 className="text-white font-bold hover:text-github-blue text-sm line-clamp-1">{issue.title}</h4>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="text-xs flex items-center gap-1.5 px-3 py-1.5 bg-github-purple/20 text-github-purple border border-github-purple/30 rounded-md hover:bg-github-purple hover:text-white transition-all disabled:opacity-50"
                    >
                      {analyzing === issue.id ? (
              ...
              {selectedIssue.aiAnalysis && (
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-github-purple/5 border border-github-purple/20 rounded-lg">
                    ...
                    <h5 className="text-white font-semibold text-xs mb-2 uppercase tracking-wide">Suggested Implementation</h5>
                    ...
                    {selectedIssue.aiProposal && (
                      <button
                        ...
                        className="w-full px-4 py-2 bg-github-blue text-white rounded-md font-semibold hover:bg-opacity-90 flex items-center justify-center gap-2"
                      >
